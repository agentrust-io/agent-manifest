...
**Two documentation tables described a conformance ladder the specification does not define.** `docs/compliance/eu-ai-act.md` listed the levels as `0 software / 1 TPM / 2 SEV-SNP or TDX / 3 managed TEE`, and the hardware-attestation tutorial mapped providers to levels the same way. Section 8.1 defines the levels by what is bound and attested, not by silicon: any TEE provider satisfies Level 1, Level 2 is all ten artifacts plus HITL, delegation, Phase 2 cMCP and retention, and Level 3 is the post-quantum profile. As written, a reader on a TPM host would have concluded they could not reach Level 2 without changing hardware, and the tutorial's own guidance table put EU AI Act Art. 15 at Level 2 where the spec puts it at Level 1. Both tables now match the spec, with the differences between roots of trust stated as what a verifier can conclude rather than as a level. `AzureCVMProvider` is added to both, since it ships and was in neither.
