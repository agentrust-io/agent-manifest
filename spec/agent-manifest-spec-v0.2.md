...
Key identifier semantics (normative): `signature.key_id` identifies the signing key but does not establish issuer authorization. A verifier MUST resolve the signing key through an authorization path external to the manifest document and MUST confirm that the resolved key is authorized for the declared `issuer` before verifying the signature. Under the v0.2 COSE envelope, `kid` identifies the signing key without establishing issuer authorization.
